...
class GetHongbaoForm extends React.Component {
	...
	renderHistoryTable = e => {
		...
		const columns = [
			{title: '领取时间', dataIndex: 'time', key: 'time', width: 60, render: renderGmtCreate},
			{title: '金额(元)', dataIndex: 'price', key: 'price', width: 60, render: renderPrice},
		...
		return <Table dataSource={this.props.historyList} columns={columns} pagination={{pageSize: 5, size: 'small', total: this.props.historyList.length}} />;
	};
			...
			<div>
				<Collapse defaultActiveKey={['1', '2', '3']} bordered={false}>
					...
					<Panel header="如何复制红包链接？" key="3">
						...
						3. 长按微信分享的卡片 - 点击更多 - 发送邮件 - 复制链接。（如果看不到链接，在微信的设置 - 通用 -
						功能 - 开启邮箱提醒）
					</Panel>
				</Collapse>
			</div>
