...
export default class Home extends React.Component {
	...
	getNotice = e => {
		axios
			.get(apis.getNotice)
			.then(data => this.setState({noticeList: data.data}));
	};
	...
	deleteCookie = id => {
		axios
			.post(apis.deleteCookie, qs.stringify({cookieId: id}))
			.then(data => {
				if (data.code === 0) {
					//前端删除
	...
	renderCarousel = e => {
		...
		let data = carouselRecords.map((o, i) => (
			<div key={i} style={{color: '#5bab60', fontSize: '16px'}}>
				{o.mail} 在 {dateFormat(new Date(o.gmtModified), 'HH:mm:ss')}{' '}
				领到
				...
				>
					<TabPane tab="规则" key="1">
						<div>
							<p>
								...
								次可消耗次数。以此类推：第 N
								个最大，最多需要消耗 N
								次。如果该红包之前已被他人领取了 M 次，则消耗 N
								- M 次。
							</p>
							<p>
								2. 每天可消耗的次数为：你贡献的 cookie 数量乘以
								5，第二天会自动回满次数，美团和饿了么不通用。
							</p>
							<p>
								3. 贡献之后的小号
							...
							<p>
								7. 美团、饿了么限制每个手机号一天只能领 5
								个红包，请确保你填写的手机号还可以领，否则会浪费你的次数。
							</p>
							<p>
								8.
							...
							<p>
								9.
								由于刚上线，可能会出现很多问题，请多多包涵。欢迎提
								issue 或者加群反馈给我们，我们也在不断修复问题。
							</p>
						</div>
					</TabPane>
					...
					<TabPane tab="贡献" key="3">
						<div style={{color: '#dd2323', marginBottom: '15px'}}>
							贡献每一个微信需要完全退出 PC 微信进程再登录小号<br />贡献每一个
							QQ 需要清除浏览器 cookie
							或打开隐身（无痕）模式再登录小号
						</div>
						<RadioGroup
						...
						<ContributeForm
							callback={this.contributeCallback}
					...
					<TabPane tab="加群" key="5" style={{textAlign: 'center'}}>
						...
						<img
							...
							src="https://note.youdao.com/yws/api/personal/file/58B17F3D8FEF477F9093D81814BDD4AA?method=download&shareKey=e4a1ab8f78b373e823eefd0cf6c5baaf"
							alt=""
						/>
					</TabPane>
				</Tabs>
	...
	renderAlipayHongbao = e => {
		return (
			<div className="alihongbao-pc">
				<img
					src={require('../../static/alipayhongbao.png')}
					width="240"
					alt="支付宝天天领红包"
				/>
				<div>
